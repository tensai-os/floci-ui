...
async function withCloudWatch(c: Context, handler: () => Promise<Response>): Promise<Response> {
    try {
        return await handler()
    } catch (err) {
        const message = err instanceof Error ? err.message : 'CloudWatch request failed'
